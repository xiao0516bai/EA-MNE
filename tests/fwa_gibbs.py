...
project = '_'.join(('FWA_gibbs_0423_4', args.ob_product))
...
def generate_explode_fireworks(chrom_list,explode_radius,explode_nums,log):
    ...
    for k in range(len(chrom_list)):
        ...
        for j in range(num):
            ...
            Original = copy.deepcopy(chrom_list[k][0]) # 原始的个体 比较一下是否跟原始的个体相同
            ...
            while (1):
                ...
                if (explode_firework_n == 10):
                    break
                explode_firework_n += 1
                if explode_middle_chrom.get_a_chrom(times=1000):
                    # 已经找到了一条 中间两个化合物的路径
                    # 先剪枝
                    ...
                    if(Chrom.chrom.length()>20):
                        ...
                        continue
                    # 如果爆炸产生的个体和原个体相同
                    elif(Chrom.chrom.get_rxn_list()==Original.chrom.get_rxn_list()):
                        continue
                    else:
                        break
