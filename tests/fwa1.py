        ...
        for j in range(num):
            ...
            while (1):
                ...
                if explode_middle_chrom.get_a_chrom(times=100):
                    ...
                    # 已经找到了一条 中间两个化合物的路径
                    # 先剪枝
                    # 这里不需要再剪枝了 因为在 get_a_chrom 中已经剪枝过了
...
args = parser()
cfg = get_config()
log = Logger(cfg['file_path']['log_dir'] + 'FWA1.log', fmt='%(message)s')
...
# pop_size= 3 # 每个子代的烟花数
# max_iter = 5
# max_iter = 50 # fwa2 在30代时就已经收敛了.设为和fwa2一致。
max_iter = 30
...
project = '_'.join(('FWA1_0415', args.ob_product))
