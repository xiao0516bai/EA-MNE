...
def evaluate(X):
    ...
    total_reactions = get_network_totalReactions(cfg,X[0],log) # 到这里的X[0] 还是有best_enzyme_pool的
    print("evaluate---------->")
    print("X[0].best_enzyme_pool")
    print(X[0].best_enzyme_pool)
    ...
    if not os.path.exists(csv_file_path):
        with open(csv_file_path, 'w', newline='') as file:
            ...
            writer.writerow(["fit", "total_reactions", "length"])
    ...
    with open(result_dir + 'all_fwa_0412.txt', 'a') as f:
    ...
    X[0].set_fit(fit)
    # 最优酶池，以及网络中的总反应等 都是在 get_network_totalReactions 函数中保存的。
...
def generate_explode_fireworks(chrom_list,explode_radius,explode_nums,log):
    ...
    for k in range(len(chrom_list)):
        ...
        log.logger.info("第 %d 个烟花发生爆炸，爆炸半径是 %d,爆炸火花数量为 %d" % (k+1, radius, num))
    ...
    with open(result_dir + 'every_generation_childs.txt', 'a') as f:
        ...
        for i in range(len(fitness)):
            ...
            for line in travel_list:
                ...
                f.write('\n')
            # # 写入最好的酶组合
            f.write(str(chrom.best_enzyme_pool))
            f.write('\n')
            # # 写入当前酶组合产生的总反应
            f.write(str(chrom.best_total_rxn_list))
            f.write('\n')
...
args = parser()
cfg = get_config()
log = Logger(cfg['file_path']['log_dir'] + 'demo.log', fmt='%(message)s')

weight1 = 0 # 长度
weight2 = 0 # 吉布斯自由能
weight3 = 0 # 理论产量
weight4 = 1 # 设置权重 weight对应的是网络最少反应数

# 我觉得这个样本数需要增加。
# 爆炸半径最小值应该设为2。因为如果爆炸半径最小值为1的话，就没有优化的可能性了。长度基本不可能会再减小了。
# 另外爆炸后的个体，判断一下是否和原个体相同。如果相同，重新爆炸。重复5次，如果5次后都是相同的，那么就以爆炸的结果为准。
...
project = '_'.join(('demo', args.ob_product))
...
# best.csv中写入第一行内容  Generation, Best Fitness
...
for idx, chrom in enumerate(chrom_list):
    ...
    fit,len0 = evaluate(chrom)   # 到这里 chrom[0].best_enzyme_pool 也是正确的
    print("fit,len0 = evaluate(chrom)----------------->")
    print("chrom[0].best_enzyme_pool")
    print(chrom[0].best_enzyme_pool)
    chrom_list[idx] = chrom
    
    print("518 ------------------->")
    print("chrom_list[0][0].best_enzyme_pool")
    print(chrom_list[0][0].best_enzyme_pool)
    fitness.append(fit)
    print("522 ------------------->")
    print("chrom_list[0][0].best_enzyme_pool")
...
with open(result_dir + 'best_childs.txt', 'a') as f:
    ...
    f.write(str([length, total_reactions, fit]))
    f.write('\n')
    # 写入路径中的反应及化合物
...
print("before save_every_generation------------------->")
print(chrom_list[0][0].best_enzyme_pool)
