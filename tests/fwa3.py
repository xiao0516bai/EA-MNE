...
from mooseeker.fit_funcs import get_gibbs, get_yield,get_network_totalReactions,get_toxicity
...
args = parser()
cfg = get_config()
log = Logger(cfg['file_path']['log_dir'] + 'FWA3.log', fmt='%(message)s')
...
project = '_'.join(('FWA3_0629', args.ob_product))
...
def evaluate(X):
    ...
    log.logger.info('--- get toxicity---')
    toxicity = get_toxicity(cfg,X[0],log,1) # 最后一个参数为1表示计算路径中化合物的毒性

    # fit = float(weight1) * length + float(weight2) * gibbs - float(weight3) * _yield
    # fit = float(weight4) * total_reactions
    fit = float(weight1) * length + float(weight5) * toxicity
